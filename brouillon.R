...
#Le code pour l'objet graphique 1
graph_1<-ggplot()+geom_sf(data=data_couche1 %>% filter(Shape_Area %in% 0:3), linetype="solid", colour="#000000", fill="#D4B9DA", linewidth=1, show.legend = "line" )  +
  geom_sf(data=st_simplify(data_couche1 %>% filter(Shape_Area %in% 4:7)), linetype="solid", colour="#000000", fill="#F46D43", linewidth=1, show.legend = "line" )  +
  coord_sf(crs=st_crs(4326),
           datum=st_crs(4326),
           label_graticule="ENWS",#la clé qui permet de generer les labels des grilles sur les 4 cotés
           xlim=c(14, 27 ),
