...
gestionnaire_objet_carte_server <- function(input, output, session, liste_objets_mise_en_page ) {
  ...
  #Paramètres généraux de position des objets du composeur###############
  ...
  observeEvent(input$name_objet_carte_select, {
    ...
    xmin_objet<- copie_liste_objets[[name_objet_select]]$xmin #eval(parse(text =  paste("copie_liste_objets", name_objet_select, "xmin", sep = "$" )   ))
    xmin_objet_actif(xmin_objet)
    xmax_objet<- copie_liste_objets[[name_objet_select]]$xmax #eval(parse(text =  paste("copie_liste_objets", name_objet_select, "xmax", sep = "$" )   ))
    xmax_objet_actif(xmax_objet)
    ymin_objet<- copie_liste_objets[[name_objet_select]]$ymin# eval(parse(text =  paste("copie_liste_objets", name_objet_select, "ymin", sep = "$" )   ))
    ymin_objet_actif(ymin_objet)
    ymax_objet<-copie_liste_objets[[name_objet_select]]$ymax# eval(parse(text =  paste("copie_liste_objets", name_objet_select, "ymax", sep = "$" )   ))
        ...
        isolate({
          tagList(#début du cadre
            ...
            fluidRow(class="form-group",
                     ...
                     column(width = 8,
                            numericInput(ns("select_PanelborderSize"), label = NULL, min = 0, max=NA, step = 0.01, width = "100px", value =  PanelborderSize )
                     )
            ),
